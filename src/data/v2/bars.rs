...
mod tests {
    ...
    fn assert_in(value: &Num, range: RangeInclusive<u64>) {
        assert!(range.contains(&value.to_u64().unwrap()), "{value} {range:?}")
    }
    ...
    async fn request_bars() {
        ...
        assert_eq!(bars.len(), 2);
        assert_eq!(bars[0].time, DateTime::<Utc>::from_str("2018-12-04T05:00:00Z").unwrap());
        ...
        assert_eq!(bars[1].time, DateTime::<Utc>::from_str("2018-12-06T05:00:00Z").unwrap());
